...
#dados_hiv <- read.dbf("Dados/hiv2.dbf")

dados_hiv <- read.dbf("//arquivos/SINAN/BaseDBF_RD/AIDSANET.dbf")

dados_hiv <- subset(dados_hiv, year(dados_hiv$DT_NOTIFIC) > 2009 & dados_hiv$CRITERIO == "901")

dados_hiv <- subset(dados_hiv, str_sub(dados_hiv$ID_MN_RESI,1,2) == "31")
...
write.xlsx(dados_hiv, "Atualizar/dados_hiv.xlsx")
...
write.xlsx(dados_hiv, "Manipulados/dados_hiv.xlsx")