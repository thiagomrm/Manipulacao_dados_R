...
dados_violencia_geral <- dados_violencia_geral %>%
  ...
  select(DT_NOTIFIC,	ID_RG_RESI, 	DT_NASC,	NU_IDADE_N,	CS_SEXO,	CS_RACA,	CS_ESCOL_N, ID_MN_RESI,	LOCAL_OCOR,	OUT_VEZES,	LES_AUTOP, SG_UF,
         ...
         DEF_TRANS,	DEF_FISICA,	DEF_MENTAL,	DEF_VISUAL,	DEF_AUDITI,	TRAN_MENT,	TRAN_COMP,	DEF_OUT,	ORIENT_SEX,	IDENT_GEN, SIT_CONJUG,) 
...
dados_violencia_geral <- subset(dados_violencia_geral, dados_violencia_geral$SG_UF == "31" & year(dados_violencia_geral$DT_NOTIFIC) > 2009)
