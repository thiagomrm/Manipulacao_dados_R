...
#dados_aids <- read.dbf("Dados/aids2.dbf")

dados_aids <- read.dbf("//arquivos/SINAN/BaseDBF_RD/AIDSANET.dbf")

dados_aids <- subset(dados_aids, year(dados_aids$DT_NOTIFIC) > 2009 & dados_aids$CRITERIO == "100" | dados_aids$CRITERIO == "300" |
                       dados_aids$CRITERIO == "600")

dados_aids <- subset(dados_aids, str_sub(dados_aids$ID_MN_RESI,1,2) == "31")

dados_aids <- dados_aids %>%
...
write.xlsx(dados_aids, "Atualizar/dados_aids.xlsx")
...
write.xlsx(dados_aids, "Manipulados/dados_aids.xlsx")